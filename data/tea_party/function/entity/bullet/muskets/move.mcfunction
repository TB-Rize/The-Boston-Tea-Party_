# tea_party:entity/bullet/muskets/move
#
...
#エンティティ衝突判定
execute at @s positioned ~-0.5 ~-0.5 ~-0.5 if entity @e[gamemode=!spectator,type=!marker,dx=0,scores={tea.bullet_resist=..0}] run function tea_party:entity/bullet/muskets/hit


#対物衝突判定
execute at @s unless block ~ ~ ~ air run function tea_party:entity/bullet/muskets/land

#再帰処理
execute if entity @s[scores={tea.bullet_Recursion=1..,tea.bullet_range=1..}] run function tea_party:entity/bullet/muskets/move

#飛距離限界に達したとき
execute if entity @s[scores={tea.bullet_range=0}] run function tea_party:entity/bullet/muskets/land
