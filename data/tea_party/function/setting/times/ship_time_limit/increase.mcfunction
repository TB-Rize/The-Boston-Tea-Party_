# tea_party:setting/times/ship_time_limit/increase
#
## @within tea_party:setting/times/ship_time_limit/page3

# 13行目の再帰でこちらに帰ってくる
execute if data storage tea_party: {setting_temp:2} run scoreboard players remove #setting_ship_time_limit_sec tea.time_limit 60
execute if data storage tea_party: {setting_temp:2} run data modify storage tea_party: setting_temp set value 1
# スコアを指定された値分増やす
$scoreboard players add #setting_ship_time_limit_$(ms) tea.time_limit $(num)

# もし秒数が60を上回ったらminの方を1上げ、secの方を60減算する
execute if score #setting_ship_time_limit_sec tea.time_limit matches 60.. run data modify storage tea_party: setting_temp set value 2
execute if data storage tea_party: {setting_temp:2} run function tea_party:setting/times/ship_time_limit/increase {num:1,ms:min}
