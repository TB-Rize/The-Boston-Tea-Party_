# tea_party:setting/times/game_time_limit/decrease
#
## @within tea_party:setting/times/game_time_limit/page1

# 16,20行目の再帰でこちらに帰ってくる
execute if data storage tea_party: {setting_temp:2} run scoreboard players add #setting_time_limit_sec tea.time_limit 60
execute if data storage tea_party: {setting_temp:3} run scoreboard players set #setting_time_limit_sec tea.time_limit 0
execute if data storage tea_party: {setting_temp:3} run scoreboard players set #setting_time_limit_min tea.time_limit 0
...
# スコアを指定された値分減らす
$scoreboard players remove #setting_time_limit_$(ms) tea.time_limit $(num)

# もし秒数が0を下回り、かつminが1以上だった時、minの方を1下げたのち負の数となっているsecに60加算する
execute if score #setting_time_limit_sec tea.time_limit matches ..-1 if score #setting_time_limit_min tea.time_limit matches 1.. run data modify storage tea_party: setting_temp set value 2
execute if data storage tea_party: {setting_temp:2} run function tea_party:setting/times/game_time_limit/decrease {num:1,ms:min}

# もし分数が0を下回ったとき、全てを0にする(numは適当に1)
execute if score #setting_time_limit_min tea.time_limit matches ..-1 run data modify storage tea_party: setting_temp set value 3
execute if data storage tea_party: {setting_temp:3} run function tea_party:setting/times/game_time_limit/decrease {num:0,ms:sec}
#execute if data storage tea_party: {setting_temp:3} run data modify storage tea_party: setting_temp set value 0

# 0を下回らないように
scoreboard players operation #setting_time_limit_min tea.time_limit > #0 tea.int
scoreboard players operation #setting_time_limit_sec tea.time_limit > #0 tea.int
